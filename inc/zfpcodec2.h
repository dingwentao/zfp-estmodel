...
public:
  ...
  inline struct RD sample(const Scalar* p, uint sx, uint sy);
...
// sample 4*4 block from p using strides
TEMPLATE struct RD CODEC2::sample(const Scalar* p, uint sx, uint sy)
{
  // convert to fixed-point
  Fixed fp[16];
  int emax = fwd_cast(fp, p, sx, sy);
...
//  printf ("minbits = %d, maxbits = %d, maxprec = %d, minexp = %d, maxexp = %d, precision(emax) = %d\n", minbits, maxbits, maxprec, minexp, emax, precision(emax));
